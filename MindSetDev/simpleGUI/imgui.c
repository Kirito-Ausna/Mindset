...
gs_menu_color = {
	"Gray",      "Black",	"Black",	    "Light gray",   1 , // 不填充
},

gs_button_color = {
	"Brown",      "White",	"Gray",	    "Light Gray",   1 , // 不填充
},

gs_textbox_color = {
	"Black",      "Black",	"Orange",	    "Orange",   0 , // 不填充
};

void setButtonColors(char *frame, char*label, char *hotFrame, char *hotLabel, int fillflag)
{
	if(frame) strcpy(gs_button_color.frame, frame);//框 
...
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	...
	if ( gs_UIState.kbdItem == id )
	{
		...
		case VK_BACK:
			if( len > 0 ) {
				if (textbuf[len-1] & 0X80 ) --len; //汉字占2个字节且每个字节最高位都为1 by xdq @20210517
		...
		if ((gs_UIState.charInput >= 32 && gs_UIState.charInput < 127 || gs_UIState.charInput & 0X80) && len+1 < buflen ) {
